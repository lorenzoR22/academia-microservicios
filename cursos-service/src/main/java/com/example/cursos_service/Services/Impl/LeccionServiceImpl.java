...
public class LeccionServiceImpl implements LeccionService {
    ...
    public LeccionResponseDTO getLeccion(Long id,String id_user) throws NotFoundException, NotInscriptionException {
        ...
        if(!inscripcionClient.existsByUserIdAndCursoId(leccion.getCurso().getId(),id_user)){
            throw new NotInscriptionException("No estas inscripto al curso de "+leccion.getCurso().getTitulo());
        }
        return leccionMapper.toDTO(leccion);
    }

    public LeccionResponseDTO saveLeccion(Long cursoId, LeccionRequestDTO dto) throws NotFoundException {
    ...
    public LeccionResponseDTO updateLeccion(Long leccionId,LeccionRequestDTO request) throws NotFoundException {
        Leccion leccion=leccionRepository.findById(leccionId)
                .orElseThrow(() -> new NotFoundException("Lección no encontrada con id: " + leccionId));
    ...
    public LeccionResponseDTO updateParcialLeccion(Long leccionId, LeccionRequestDTO request) throws NotFoundException {
        Leccion leccion = leccionRepository.findById(leccionId)
                .orElseThrow(() -> new NotFoundException("Lección no encontrada con id: " + leccionId));
